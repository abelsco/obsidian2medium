...
import { checkStatusFetch, HTTPResponseError } from '../errors/httpError.mjs'
...
export class MediumAPI {
  ...
  */
  ...
  /**
  * Check status response from external call
  *
  * @param {Number} status code status from response
  * @returns {Error} If status equals to 400|401|404 throw a Error
  */
  checkStatus(status) {
    switch (status) {
      case 400:
        throw new Error('Verify your connection')
      case 401:
        throw new Error('Cannot get your authentication from Medium, verify your .env')
      case 404:
        throw new Error('Cannot connect Medium API, verify your .env')
      default:
        break;
    }
  }
